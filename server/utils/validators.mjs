...
export const notificationPreferencesSchema = z.object({
  ...
  notifyLeadManaged: z.boolean().default(true),
  notifyLeadConverted: z.boolean().default(true),
  notifyAiFailed: z.boolean().default(true),
  cadenceMode: z.enum(['immediate', 'hourly', 'daily', 'custom']).default('immediate'),
  cadenceIntervalMinutes: z.number().int().min(5).max(1440).optional().nullable(),
  maxNotificationsPerDay: z.number().int().min(1).max(200).optional().nullable(),
  quietHoursStart: z.string().regex(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/, 'Invalid time format').optional().nullable(),
  quietHoursEnd: z.string().regex(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/, 'Invalid time format').optional().nullable(),
  timezone: z.string().min(2).max(64).default('Europe/Copenhagen'),
